...
int32_t execute_aes_circuit(e_role role, char* address, uint16_t port, uint8_t* inval, uint8_t** outval, seclvl seclvl, uint32_t nvals, uint32_t nthreads,
	...
	input.CreateBytes(AES_BITS * nvals);
	if(role == CLIENT) {
        input.Copy(inval, 0, nvals*AES_BYTES);
	}
	...
	if(sharing == S_YAO_REV) {
        ...
        if(role == CLIENT)
            *outval = s_ciphertext_yao->get_clear_value_ptr();

		if(nyao_rev_circs > 0) {
            if(role == CLIENT)
                *outval = s_ciphertext_yao_rev->get_clear_value_ptr();
		}
