...
//    printhexblock("input", message);
    block c = message ^ roundkeys[0];
//    printhexblock("keyadd", c);
    for (unsigned r = 1; r <= rounds; ++r) {
        c =  Substitution(c);
//        if(r == r) printhexblock("sub", c);
        c =  MultiplyWithGF2Matrix(LinMatrices[r-1], c);
//        if(r == r) printhexblock("lin", c);
        c ^= roundconstants[r-1];
//        if(r == r) printhexblock("rc", c);
        c ^= roundkeys[r];
//        if(r == r) printhexblock("rk", c);
    }
    return c;
}
...
void LowMC::lowmc_set_key (keyblock k) {
    key = k;
    keyschedule();
}
...
block LowMC::Substitution (const block message) {
    ...
    for (unsigned i = 1; i <= numofboxes; ++i) {
        temp <<= 3;
//        std::cout << ((message >> 3*(numofboxes-i))& block(0x7)).to_ulong()  << std::endl;
